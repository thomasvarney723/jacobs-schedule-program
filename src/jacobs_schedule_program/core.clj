...
(defn -main []
  (print \tab "Which of the following tasks would you like completed?:" nl
         nl "\t\t" "1) Import roster of team member and their schedules"
         nl "\t\t" "2) Print the roster of team members"
         nl "\t\t" "3) Print a team member's Schdedule"
         nl "\t\t" "4) Print an overview of the coordinator's schedule"
         nl "\t\t" "5) Print an overview of the load puller's schedule"
         nl "\t\t" "6) Print an overview of the department's schedule"
         nl "\t\t" "7) Quit" nl nl
         \tab "What is the number choice of your desired selection?: ")
  (flush)
  (let [in (read-line)]
    (case in
      "1" (do (print (str nl "\t\t" "You chose to generate the roster of team members and their schedules."
                          nl nl "\t\t" \tab "What file should we import team member data from? : "))
              (flush)
              (let [in (read-line)]
                (def data (structure-data (slurp in)))
                (print (str nl "\t\t" "You've successfully created the roster!" nl nl))
                (recur)))
      "2" (do (run! print (concat "\n" "\t\t" "You chose to print the roster of team members."
                                  ...
                                  (mapv #(str nl "\t\t" \tab "The name of team member " %1 " is " (:name %2) "." nl)
                                        (range 1 (inc (count data)))
                                        data)
                                  "\n" "\t\t" "The roster of team members can be seen above." "\n" "\n"))
              (recur))
      "3" (do (print (str nl "\t\t" "You chose to print a team member's schedule!" nl "\t\t"
                          "What team member would you like to see the schedule of? (Enter their position in the roster): "))
              (flush)
              (let [in (read-line)]
                (print (str nl "\t\t" "Their schedule looks like this: " nl))
                (run! println (->> in Integer. dec (nth data) :weeks make-table))
                (recur)))
      "4" (do (print (str nl "\t\t" "You chose to print the coordinator's schedule!"
                          nl "\t\t" "Their schedule looks like this: " nl))
              (run! println (->> data (filter #(= "1" (:status %))) sum-team-members make-table))
              (recur))
      "5" (do (print (str nl "\t\t" "You chose to print the load puller's schedule!"
                          nl "\t\t" "Their schedule looks like this: " nl))
              (run! println (->> data (filter #(= "2" (:status %))) sum-team-members make-table))
              (recur))
      "6" (do (print (str nl "\t\t" "You chose to print the entire department schedule!"
                          nl "\t\t" "Their schedule looks like this: " nl))
